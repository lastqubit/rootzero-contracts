...
library Erc721Cursors {
    ...
    /// @notice Validate a CUSTODY_AT block for a specific local ERC-721 collection and return its metadata.
    /// Reverts if the block is not CUSTODY_AT, the host differs, the asset is not a local ERC-721, the collection differs, or the amount is not 1.
    /// @param cur Source cursor.
    /// @param i Byte offset of the CUSTODY_AT block.
    /// @param host Expected host node ID from the block.
    /// @param collection Expected local ERC-721 collection.
    /// @return meta Asset metadata slot from the block.
    function expectErc721CustodyAt(
        Cur memory cur,
        uint i,
        uint host,
        address collection
    ...
    /// @notice Consume a CUSTODY_AT block for a specific local ERC-721 collection and return its metadata.
    /// Reverts if the current block is not CUSTODY_AT, the host differs, the asset is not a local ERC-721, the collection differs, or the amount is not 1.
    /// @param cur Cursor; advanced past the block.
    /// @param host Expected host node ID from the block.
    /// @param collection Expected local ERC-721 collection.
    /// @return meta Asset metadata slot from the block.
    function requireErc721CustodyAt(Cur memory cur, uint host, address collection) internal view returns (bytes32 meta) {
        meta = expectErc721CustodyAt(cur, cur.i, host, collection);
        cur.i += 136;
    }
}
