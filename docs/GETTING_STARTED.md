...
This guide is for developers who want to build on Rush without reading the whole codebase first.

If you remember only one thing, remember this:

- A `Host` is your application contract.
- A command is an entrypoint the Rush runtime is allowed to call.
- Requests and responses are passed around as typed byte blocks.

## The Mental Model

Rush moves data through a small command context:
...
The smallest useful Rush app is a host contract.
...
- you get the standard Rush command surface
...
const ctx = {
  target: 0n,
  account: "0x...", // 32-byte Rush account id
  state: "0x",
...
import {Host} from "rush/contracts/Core.sol";
import {CommandContext} from "rush/contracts/Commands.sol";
import {toCommandId} from "rush/contracts/Utils.sol";
...
- keep the rest of the Rush request format unchanged
...
import {Writers} from "rush/contracts/Blocks.sol";
import {Writer} from "rush/contracts/Schema.sol";
