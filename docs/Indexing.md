...
| `end` native refund        | `Received` | `Actions.Refund` |
...
The `Received/Refund` event from `end` is emitted when the refund
...
- **Unconditional library-forced flow emission for hook-driven commands.** Hosts
  ...
  `end` is different: the library itself drains the budget and creates
