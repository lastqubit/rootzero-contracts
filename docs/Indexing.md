...
TRANSACTION block is created, before the pipeline host posts that block. It
...
posting-side balance or flow event. Synchronous EVM execution remains atomic:
if posting or a later pipeline step reverts, the refund event is reverted as
well.
