...
balance. The protocol-owned exception is `Payable.closeValue`, which emits
...
| closeValue native refund   | `Received` | `Actions.Refund` |
...
The `Received/Refund` event from `closeValue` is emitted when the refund
TRANSACTION block is created, before the pipeline host settles that block. It
describes the refund generated by the command, not independent proof that the
destination ledger was credited. Indexers that need completion should use the
settlement-side balance or flow event. Synchronous EVM execution remains atomic:
if settlement or a later pipeline step reverts, the refund event is reverted as
well.
...
Mint 7, Burn 8, Swap 9, Borrow 10, Repay 11, Liquidate 12, Refund 13
```
...
- **Unconditional library-forced flow emission for hook-driven commands.** Hosts
  ...
  `closeValue` is different: the library itself drains the budget and creates
  the exact refund transaction, so it can emit the matching `Received/Refund`
  event without guessing host policy.
