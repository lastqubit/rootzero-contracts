...
abstract contract MyCommand is CommandBase {
    bytes4 private immutable inputKey = Keys.local(1);
    bytes32 private constant inputName = bytes32("sendToHost");
    string private constant inputSchema = "{ uint host, #amount as amount }";

    bytes32 private immutable descriptor;

    constructor() {
        // CUSTODIES = this command returns CUSTODY blocks.
        (, descriptor) = command("myCommand", Keys.Empty, schema(inputKey, inputSchema, inputName), Keys.Custody, 0, false, false);
    }
    ...
    function unpackInput(Cur memory input) private view returns (uint targetHost, bytes32 asset, uint amount) {
        (uint abs, uint next) = input.expect(input.i, 0, inputKey, 32, 0);
