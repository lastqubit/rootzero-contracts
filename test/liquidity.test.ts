...
describe("Liquidity Commands", () => {
  ...
  it("addLiquidityFromCustodiesToBalances maps a custody pair and parses the route minimum", async () => {
    ...
    const route = "0x1234";
    const request = encodeRouteBlockWithMinimum(route, MIN_ASSET, MIN_META, MIN_AMOUNT);
    ...
    await expect(tx).to.emit(host, "AddCustodiesMapped")
      .withArgs(userAccount, assetA, 10n, assetB, 20n, route);
    ...
    expect(result).to.equal(concat(
      encodeBalanceBlock(assetA, META, 12n),
      encodeBalanceBlock(assetB, META, 23n),
      encodeBalanceBlock(LP_FROM_CUSTODIES_ASSET, pad32(2n), 30n),
    ));
  });
  ...
  it("addLiquidityFromCustodiesToBalances batches multiple custody pairs", async () => {
    ...
    const request = concat(
      encodeRouteBlock("0x11"),
      encodeRouteBlock("0x2233"),
    );
    ...
    const result: string = await (host as any)[addCustodiesMethod].staticCall(ctx({ state, request }));
    expect(result).to.equal(concat(
      encodeBalanceBlock(assetA, META, 11n),
      encodeBalanceBlock(assetB, META, 22n),
      encodeBalanceBlock(LP_FROM_CUSTODIES_ASSET, pad32(1n), 30n),
      encodeBalanceBlock(assetC, META, 32n),
      encodeBalanceBlock(assetD, META, 43n),
      encodeBalanceBlock(LP_FROM_CUSTODIES_ASSET, pad32(2n), 70n),
    ));
  });
  ...
  it("removeLiquidityFromCustodyToBalances maps a custody input and returns two balances", async () => {
    ...
    await expect(tx).to.emit(host, "RemoveCustodyMapped")
      .withArgs(userAccount, asset, 15n, route);
    await expect(tx).to.emit(host, "MinimumObserved")
      .withArgs(MIN_ASSET, MIN_META, MIN_AMOUNT);

    const result: string = await (host as any)[removeCustodyMethod].staticCall(ctx({ state, request }));
    expect(result).to.equal(concat(
      encodeBalanceBlock(asset, META, 17n),
      encodeBalanceBlock(REDEEM_FROM_CUSTODY_ASSET, pad32(2n), 25n),
    ));
  });
  ...
  it("removeLiquidityFromCustodyToBalances batches multiple custody inputs", async () => {
    ...
    const result: string = await (host as any)[removeCustodyMethod].staticCall(ctx({ state, request }));
    expect(result).to.equal(concat(
      encodeBalanceBlock(assetA, META, 16n),
      encodeBalanceBlock(REDEEM_FROM_CUSTODY_ASSET, pad32(1n), 25n),
      encodeBalanceBlock(assetB, META, 27n),
      encodeBalanceBlock(REDEEM_FROM_CUSTODY_ASSET, pad32(2n), 35n),
    ));
  });
  ...
  it("addLiquidityFromBalancesToBalances maps a balance pair", async () => {
    ...
    await expect(tx).to.emit(host, "AddBalancesMapped")
      .withArgs(userAccount, assetA, 9n, assetB, 19n, route);
    await expect(tx).to.emit(host, "MinimumObserved")
      .withArgs(MIN_ASSET, MIN_META, MIN_AMOUNT);

    const result: string = await (host as any)[addBalancesMethod].staticCall(ctx({ state, request }));
    expect(result).to.equal(concat(
      encodeBalanceBlock(assetA, META, 10n),
      encodeBalanceBlock(assetB, META, 22n),
      encodeBalanceBlock(LP_FROM_BALANCES_ASSET, pad32(1n), 28n),
    ));
  });
  ...
  it("addLiquidityFromBalancesToBalances batches multiple balance pairs", async () => {
    ...
    const request = concat(
      encodeRouteBlock("0x55"),
      encodeRouteBlock("0x6677"),
    );
    ...
    expect(result).to.equal(concat(
      ...
      encodeBalanceBlock(assetB, META, 22n),
      encodeBalanceBlock(LP_FROM_BALANCES_ASSET, pad32(1n), 28n),
      encodeBalanceBlock(assetC, META, 31n),
      encodeBalanceBlock(assetD, META, 43n),
      encodeBalanceBlock(LP_FROM_BALANCES_ASSET, pad32(2n), 68n),
    ));
  });
  ...
  it("removeLiquidityFromBalanceToBalances maps a balance input and returns two balances", async () => {
    ...
    await expect(tx).to.emit(host, "RemoveBalanceMapped")
      .withArgs(userAccount, asset, 17n, route);
    await expect(tx).to.emit(host, "MinimumObserved")
      .withArgs(MIN_ASSET, MIN_META, MIN_AMOUNT);

    const result: string = await (host as any)[removeBalanceMethod].staticCall(ctx({ state, request }));
    expect(result).to.equal(concat(
      encodeBalanceBlock(asset, META, 19n),
      encodeBalanceBlock(REDEEM_FROM_BALANCE_ASSET, pad32(2n), 37n),
    ));
  });
  ...
  it("removeLiquidityFromBalanceToBalances batches multiple balance inputs", async () => {
    ...
    const result: string = await (host as any)[removeBalanceMethod].staticCall(ctx({ state, request }));
    expect(result).to.equal(concat(
      encodeBalanceBlock(assetA, META, 18n),
      encodeBalanceBlock(REDEEM_FROM_BALANCE_ASSET, pad32(1n), 37n),
      encodeBalanceBlock(assetB, META, 29n),
      encodeBalanceBlock(REDEEM_FROM_BALANCE_ASSET, pad32(2n), 47n),
    ));
  });
